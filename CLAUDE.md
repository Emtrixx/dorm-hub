...
- Roles: `User.roles` is an array of domain roles (`news`, `wiki`, `hubs`; `admin` implies all). `auth.requireRole('<role>')` goes after `requireJWT` — app.js applies it router-wide for `/news/secure` and `/wiki/secure`; hub management routes in `routes/blackboard/secure.js` apply it per-route (other blackboard secure routes only need login). Roles are re-read from the DB on every request (the JWT strategy loads the user), so changes apply without re-login. Assign roles with `node scripts/set-roles.js <email> <role,role|none>`; the login response includes `roles` for client UI gating (`useAuthStore().hasRole(role)` — display only, never enforcement).
